...
public class GenerateReportServlet extends HttpServlet {
	...
	public static class TableModelDecorator implements TableModel {
		...
		private boolean format = false; // format or no the values. If format =
										// true, all values to the report are
										// String
		private Integer columnCountLimit;

		public TableModelDecorator(HttpServletRequest request, TableModel original, List metaProperties, Locale locale,
				boolean labelAsHeader, boolean format, Integer columnCountLimit) throws Exception {
		...
		public String getColumnName(int c) {
			return labelAsHeader ? getMetaProperty(c).getLabel(locale)
		...
		public Object getValueAt(int row, int column) {
			if (isFormat())
				...
				return getValueWithoutWebEditorsFormat(row, column);
		}

		private Object getValueWithoutWebEditorsFormat(int row, int column) {
			Object r = original.getValueAt(row, column);

			if ("PASSWORD".equals(getMetaProperty(column).getStereotype())) {
				r = "**************";
			}
			if (r instanceof Boolean) {
				if (((Boolean) r).booleanValue())
					return XavaResources.getString(locale, "yes");
			...
			if (r instanceof java.util.Date) {
				MetaProperty p = getMetaProperty(column); // In order to use the
															// type declared by
															// the developer
		...
		private Object getValueWithWebEditorsFormat(int row, int column) {
			...
			if ("PASSWORD".equals(metaProperty.getStereotype())) {
				r = "**************";
			}
			String result = WebEditors.format(this.request, metaProperty, r, null, "", true);
			if (isHtml(result)) { // this avoids that the report shows html
									// content
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (Users.getCurrent() == null) { // for a bug in websphere portal
												// 5.1 with Domino LDAP
			...
			if (uri.endsWith(".pdf")) {
				...
				synchronized (tab) {
					tab.setRequest(request);
					response.getWriter().print(TableModels
							.toCSV(getTableModel(request, tab, selectedRows, true, false, columnCountLimit)));
				}
			} else if (uri.endsWith(".xls")) {
				synchronized (tab) {
					tab.setRequest(request);
					JxlsWorkbook wb = new JxlsWorkbook(
							getTableModel(request, tab, selectedRows, true, false, columnCountLimit), getFileName(tab));
					JxlsSheet sheet = wb.getSheet(0);
					int lastRow = sheet.getLastRowNumber();
					JxlsStyle sumStyle = wb.addStyle(JxlsConstants.FLOAT).setBold().setBorder(JxlsConstants.TOP,
							JxlsConstants.BORDER_THIN);
					for (int column = 0; column < tab.getTableModel().getColumnCount(); column++) {
						MetaProperty property = tab.getMetaProperty(column);
						if (!property.isNumber() || !tab.hasTotal(column))
							continue;
						sheet.setFormula(column + 1, lastRow + 1,
								"=SUM(R2C" + (column + 1) + ":R" + lastRow + "C" + (column + 1) + ")", sumStyle);
					}
					wb.write(response);
				}
			} else {
	...
	private InputStream getReport(HttpServletRequest request, HttpServletResponse response, Tab tab,
			TableModel tableModel, Integer columnCountLimit) throws ServletException, IOException {
	...
	private int[] getWidths(TableModel tableModel) {
		...
		for (int r = 0; r < Math.min(tableModel.getRowCount(), 500); r++) { // 500
																			// is
																			// not
																			// for
																			// performance,
	...
	private TableModel getTableModel(HttpServletRequest request, Tab tab, int[] selectedRows, boolean labelAsHeader,
			boolean format, Integer columnCountLimit) throws Exception {
		...
		return new TableModelDecorator(request, data, tab.getMetaProperties(), Locales.getCurrent(), labelAsHeader,
				format, columnCountLimit);
	}
	...
	private int[] getSelectedRows(int[] selectedRowsNumber, Map[] selectedRowsKeys, Tab tab) {
		if (selectedRowsKeys == null || selectedRowsKeys.length == 0)
			return new int[0];
		// selectedRowsNumber is the most performant so we use it when possible
		else if (selectedRowsNumber.length == selectedRowsKeys.length)
			return selectedRowsNumber;
		else {
			// find the rows from the selectedKeys

			// This has a poor performance, but it covers the case when the
			// selected
			// rows are not loaded for the tab, something that can occurs if the
			// user
